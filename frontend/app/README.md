# Interface — Slides (React + TypeScript + Vite)

Interface para organizar os documentos de slides por **tipo** (HTML e CSS / JavaScript) e **categoria** (Slides, README, Relatório). Design clean e responsivo.

## Como rodar

- **Desenvolvimento:** `npm run dev` (abre em `http://localhost:5173`).
- **Build:** `npm run build`. A saída fica em `app/dist/`.
- **Visualizar o build:** `npm run preview` ou abrir `app/dist/index.html` no navegador (a partir da raiz do repositório, os links para os `.md` usam `../../` e funcionam).
